...
// ---------------------------------------------------------------------------
// collections / bookmarks / tags (6.4.6)
// ---------------------------------------------------------------------------
...
export async function createCollection(userId, name, description) {
  await delay(150)
  return withDb((db) => {
    const dup = db.collections.some(
      (c) => c.user_id === userId && c.name === name && !c.deleted_at
    )
    if (dup) throw new ApiError(409, '이미 같은 이름의 컬렉션이 있습니다. (ux_collections_user_name)')
    const collection = {
      id: nextId(db, 'collections'),
      user_id: userId,
      name,
      description: description || null,
      sort_order: db.collections.filter((c) => c.user_id === userId).length,
      created_at: nowIso(),
      deleted_at: null,
    }
...
export async function deleteCollection(collectionId, userId) {
  await delay(120)
  return withDb((db) => {
    const c = db.collections.find((c) => c.id === collectionId && c.user_id === userId)
    if (!c) throw new ApiError(404, '컬렉션을 찾을 수 없습니다.')
    c.deleted_at = nowIso()
    db.bookmarks
      .filter((b) => b.collection_id === collectionId)
      .forEach((b) => (b.deleted_at = nowIso()))
  })
}

export async function listBookmarks(collectionId) {
  await delay()
  return readDb((db) => {
    const bookmarks = db.bookmarks
      .filter((b) => b.collection_id === collectionId && !b.deleted_at)
      .sort((a, b) => new Date(b.created_at) - new Date(a.created_at))
    return bookmarks.map((b) => ({
      ...b,
      tags: db.bookmark_tags
        .filter((bt) => bt.bookmark_id === b.id)
        .map((bt) => db.tags.find((t) => t.id === bt.tag_id))
        .filter(Boolean),
    }))
  })
}

export async function createBookmark({ collectionId, userId, url, title, memo, tagNames = [] }) {
  await delay(200)
  if (!URL_SCHEME_PATTERN.test(url)) {
    throw new ApiError(400, 'http:// 또는 https:// 로 시작하는 URL만 입력할 수 있습니다.')
  }
  const bookmark = withDb((db) => {
    const b = {
    ...
    for (const rawName of tagNames) {
      ...
      if (!name) continue
      let tag = db.tags.find((t) => t.user_id === userId && t.name === name)
      if (!tag) {
        tag = { id: nextId(db, 'tags'), user_id: userId, name }
        db.tags.push(tag)
      }
      db.bookmark_tags.push({ bookmark_id: b.id, tag_id: tag.id })
    }
    return b
  })

  // OG 태그 크롤링 비동기 처리 시뮬레이션 (문서 6.4.6 meta_status PENDING -> DONE)
  setTimeout(() => {
    withDb((db) => {
      const b = db.bookmarks.find((x) => x.id === bookmark.id)
      if (!b) return
      b.meta_status = 'DONE'
      b.description = b.description || `${new URL(b.url).hostname} 페이지 미리보기 (데모)`
    })
    window.dispatchEvent(
      new CustomEvent('linkshortener:bookmark-updated', { detail: { bookmarkId: bookmark.id } })
    )
  }, 1500)
...
export async function deleteBookmark(bookmarkId) {
  await delay(120)
  return withDb((db) => {
    const b = db.bookmarks.find((x) => x.id === bookmarkId)
    if (b) b.deleted_at = nowIso()
  })
}

export async function listTags(userId) {
  await delay(100)
  return readDb((db) => db.tags.filter((t) => t.user_id === userId))
}
