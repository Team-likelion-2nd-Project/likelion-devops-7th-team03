 ...
 * StatsService가 SecurityContext의 principal(JWT subject UUID)로 UserRepository를 조회해
 * 이 내부 id를 얻으므로, 아래 setUp()에서 실제 JwtAuthenticationFilter와 동일한 방식으로
 * SecurityContextHolder에 그 UUID를 심어준다 (@WithMockUser는 principal이 문자열이
 * 아니라서 StatsService의 (String) 캐스팅과 안 맞아 못 씀).
...
class StatsControllerIntegrationTest {
    ...
    void setUp() {
        ...
        //    StatsService.resolveCurrentUserId()가 principal(String)로 UserRepository를 조회한다.
